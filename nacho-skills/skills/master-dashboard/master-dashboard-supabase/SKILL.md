...
description: >
  Composer for the 7-tab Sophie Master Dashboard (Daily / Negatives / Push / Review / Harvest / Restock / Optimizations).
  Reads EVERYTHING from Supabase: the dashboard_snapshots (by type) and the template from the
  dashboards table (id='master'). Rebuilds the 7-tab DATA, injects ASCII-safe, validates the
...
# Master Dashboard — Supabase → GitHub Pages (compose & publish, 7 tabs)

**Versión:** V2.4 (2026-09-08) — **agrega el tab Optimizations** (`DATA.optimizations`), alimentado desde los snapshots `tipo='optimizations'` (schema `optimizations-v1`, escritos por `biweekly-bid-optimizer`) y renderizado por `renderOptimizations()` en el template (ya inyectado 2026-09-08). Ventana de 7 días → `days[]` por cliente (cubre las corridas Tue+Fri + history aplicada de la semana), igual patrón que Push/Review. El template keyea cada fila de propuesta por `(entity_id, bidding_entity)` (los placements comparten `entity_id`). **También** agrega una **normalización defensiva de brand names** (limpia `''`→`'` en todos los tabs) porque algunos feeders escriben el brand con apóstrofo doble y duplicaban la marca en el selector. Antes: V2.3 (2026-09-06) — pasa `held_low_confidence` (del recibo `negatives_push` V2 del autopush) al `DATA.push.clients[].days[]`, para que el template renderice la **cola de aprobación de LOW** (checkboxes + "copiar aprobados"). Ver `relevance-profiles-hardening/05-dashboard.md` (renderer `renderApprovalQueue`) y `04-daily-negatives-autopush`. Antes: V2.2 (2026-09-04). Igual que V2.1, pero **agrega el tab Review** (`DATA.review`), alimentado desde los snapshots `tipo='negatives_review'` (escritos por `weekly-negatives-review`, schema `negatives-review-v1`) y renderizado por `renderReview()` en el template. Antes el Weekly Review escribía a Supabase pero el composer no lo leía → no había superficie que lo mostrara. V2.1 restauró el tab Push (`DATA.push`) que había quedado sin alimentar; ver `memory/dashboard_cloud_migration.md`.
...
> **Regression guard (obligatorio antes de publicar):** el `DATA` compuesto debe tener las 8 keys `meta, daily, negatives, push, review, harvest, restock, optimizations`. Si falta cualquiera → **STOP, no publiques** — es exactamente el bug del 2026-09-02 (tab Push vacío) repitiéndose.
...
```
dashboard_snapshots  → los 7 feeds por tipo: 'daily_check' | 'negatives' | 'negatives_push' | 'negatives_review' | 'harvest' | 'restock' | 'optimizations'
...
**1a — las 6 fuentes + el link del daily** (un solo query):
```sql
select json_build_object(
  ...
  'optimizations', (select coalesce(json_agg(json_build_object('cliente',cliente,'datos',datos,'fecha',fecha)),'[]'::json)
                    from (select cliente, datos, fecha from public.dashboard_snapshots
                          where tipo='optimizations' and fecha >= (current_date - 7)
                          order by cliente, fecha desc) o),
...
**Nota sobre `optimizations`:** las filas de `tipo='optimizations'` (escritas por `biweekly-bid-optimizer`, schema `optimizations-v1`) son **planas** (como push/review): keys al mismo nivel `schema, generated_at_iso, brand, marketplace, preset_used, window_days, tacos_used, activity, status, counts, campaigns[], proposal[], preview_params`. El `proposal[]` usa un shape **compacto** con lookup de campañas: cada fila `{id, be, kind, ci (índice en campaigns[]), tgt, old, new, why?, cap?, ag?, raw?}` — **NO** la desarmes ni la re-mapees, pasala tal cual (el template `renderOptimizations()` la consume así). Corre **bi-weekly (Tue/Fri)** → hasta 2 filas por semana por cliente; el Step 2 las reagrupa a `{brand_name, marketplace, days[]}` (cada `day` = una corrida, con su `fecha`/`status`). `preview_params` queda dentro del `day` para el apply-flow; el dashboard no lo usa pero no lo dropees.
        ...
        "days": [
            {
                ...
                "held": s.get("held", []),
                "held_low_confidence": s.get("held_low_confidence", []),   # V2 autopush: gate → cola de aprobación LOW
...
# ── OPTIMIZATIONS (últimos 7 días por cliente → reconstruir days[]) ──
# Filas planas (tipo='optimizations', schema optimizations-v1): keys al mismo nivel; bi-weekly (Tue/Fri).
# proposal[] va COMPACTO (id/be/kind/ci/tgt/old/new/why/cap/ag/raw) — pasarlo tal cual.
opt_by_client = OrderedDict()   # rows ordenadas por cliente, fecha desc (más nuevo primero)
for r in payload.get("optimizations", []):
    opt_by_client.setdefault(r["cliente"], []).append(r)
optimizations_clients = []
for cli, rows in opt_by_client.items():
    newest = rows[0]["datos"]
    optimizations_clients.append({
        "brand_name": newest.get("brand", cli),
        "marketplace": newest.get("marketplace", ""),
        "days": [
            {
                "fecha": rw.get("fecha"),
                "generated_at_iso": rw["datos"].get("generated_at_iso"),
                "status": rw["datos"].get("status", "pending"),
                "marketplace": rw["datos"].get("marketplace", ""),
                "preset_used": rw["datos"].get("preset_used"),
                "window_days": rw["datos"].get("window_days"),
                "tacos_used": rw["datos"].get("tacos_used"),
                "counts": rw["datos"].get("counts", {}),
                "campaigns": rw["datos"].get("campaigns", []),
                "proposal": rw["datos"].get("proposal", []),
                "preview_params": rw["datos"].get("preview_params", {}),
            }
            for rw in rows
        ],
    })
...
# ── Normalización defensiva de brand names (blindaje anti apóstrofo-doble) ──
# Algunos feeders escriben el brand con apóstrofo doble (ej. "Dr. Cohen''s Acuball") por un bug
# de escapado → duplicaba la marca en el selector. Acá lo limpiamos siempre, pase lo que pase.
for _lst in (daily_clients, neg_clients, push_clients, review_clients,
             harvest_clients, restock_clients, optimizations_clients):
    for _c in _lst:
        if isinstance(_c.get("brand_name"), str):
            _c["brand_name"] = _c["brand_name"].replace("''", "'")

# ── DATA de 6 tabs (misma forma que el master original + push + optimizations) ─
DATA = {
    ...
    "optimizations": {"clients": optimizations_clients},
}
...
assert set(DATA.keys()) == {"meta", "daily", "negatives", "push", "review", "harvest", "restock", "optimizations"}, \
    f"DATA le falta una tab — keys: {sorted(DATA.keys())}"
open(OUT, "w", encoding="utf-8").write(html)
print(f"OK daily={len(daily_clients)} neg={len(neg_clients)} push={len(push_clients)} review={len(review_clients)} harvest={len(harvest_clients)} restock={len(restock_clients)} opt={len(optimizations_clients)}")
```
...
Confirmá: `"Master published to GitHub Pages: {D} daily · {N} negatives · {P} push · {RV} review · {H} harvest · {R} restock · {O} optimizations → https://ignaciobosca.github.io/sophie--Ignacio-dashboard/ (commit {hash})"`.
